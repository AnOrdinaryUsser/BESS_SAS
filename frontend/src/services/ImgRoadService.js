...
export const uploadImage = async (imageData) => {
  try {
    ...
    formData.append('userName', imageData.userName);
    formData.append('image', imageData.image, imageData.name);
    formData.append('name', imageData.name);

    console.log(imageData.userName);
 ...
 */
export const getAllImages = async (userName) => {
  try {
    console.log("RESPONSE: ", `${API_URL}/getAllImages/${userName}`)
    const response = await axios.get(`${API_URL}/getAllImages/${userName}`);
 ...
 */
export const deleteImage = async (userName, imageName) => {
  try {
    const response = await axios.delete(`${API_URL}/deleteImage/${userName}/${imageName}`);
    return response.data; // Devolver la respuesta del servidor si es necesario
  } catch (error) {
    console.error(error);
    throw error;
